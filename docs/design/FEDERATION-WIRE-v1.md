...
- **Conflict envelope.** `ConflictError` → **HTTP 409** with body `{error, key, expected_base, actual_base}` so the client can re-read and retry. `SchemaSkewError` → **409** (semantic reject). The status codes are part of this LOCKED contract (so #154/#155 wire tests are deterministic).
