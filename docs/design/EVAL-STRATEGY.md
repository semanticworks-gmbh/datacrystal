...
| **4** | **MaStR** (`mastr.py`, dl-de/by-2.0) — German energy register, **SOR/metadata at vision SCALE** | 500,000 solar units (capped; full corpus ~22 GB / millions) · **18k units/s** · 201 B/unit · streaming iterparse + batched commits | the **first vision-scale-direction number** + the load-bearing proof: **peak RSS 548 MB bounded by the batch (50k), NOT the corpus**; bitmap queries on 500k records sub-2 ms (Bayern 122,082 in 1.7 ms); **#13 multi-valued** list membership == full-scan oracle (0.04 ms) on real comma-separated data |
...
**What #4 adds beyond #1–#3:** scale. The first multi-GB-direction run — streaming `iterparse` + batched commits ingest 500k real solar units (the full corpus is ~22 GB / millions, one env flag away: `MASTR_MAX=0`) with **peak RSS bounded by the commit batch, not the corpus** — the load-bearing proof that the streaming/batched-commit path holds at scale (the honest gap the earlier grounds left open). It also lands the SOR/metadata persona (indexed `Bundesland`/`PLZ`/`Betriebsstatus`) and the **#13 multi-valued list index** on genuinely comma-separated real data, both proven against full-scan oracles. The absolute throughput (~18k units/s here) is bounded by the known commit-path cost (the P1 graph-walk, already a benchmark warn-stage) — the clearest signal yet of where lib perf-work would pay off.
