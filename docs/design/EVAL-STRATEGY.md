...
| **3** | **deps.dev** (`deps_dev.py`, CC-BY) — npm dependency graph, **the CYCLIC graph (#29 reproducer)** | 747 package-versions · 1,484 edges · auto-fetched + cached | a real **cycle survives persist+reopen** (`resolve-dir→global-modules→resolve-dir`) and is followed **back to the SAME live instance** ✓ (identity through a cycle); lazy closure walk depth 11 hydrates **311 of 747**, **no `RecursionError`**; `incoming()` = dependents == full scan **exactly** |
| **4** | **MaStR** (`mastr.py`, dl-de/by-2.0) — German energy register, **SOR/metadata at vision SCALE** | **6,200,022 solar units (FULL corpus, ~22 GB XML)** · **15.6k units/s** · 1.15 GB on disk · 185 B/unit · streaming iterparse + batched commits | the **first true multi-million-record number**; bitmap queries on 6.2M records **1.4 ms** (Bayern 1,395,986); **#13 multi-valued** == oracle (0.05 ms). **Gap it caught:** ingest **peak RSS 3.9 GB is NOT batch-bounded** — the in-RAM unique/index maps grow with the corpus (the entity working set is bounded; the indexes aren't), and throughput degrades 18k→15.6k/s as they grow. Cold reopen (O(corpus) index rebuild) = 31.9 s. → filed as a perf/scale finding |
| **5** | **BEIR NFCorpus** (`search.py`, CC-BY-SA) — full-text search, **the first RELEVANCE oracle** | 3,633 docs · FTS build 356 docs/s · 26 MB sidecar · 24 ms/query | the first **ranking-quality** numbers vs *human qrels* (not exact-set equality): nDCG@10 **0.209** · P@10 0.140 · R@100 0.104 · MRR 0.350. **Gap it caught:** below the ~0.325 BM25 baseline — `[fts]` **AND's all query terms**, so a natural-language question needs *every* term in one doc → recall collapses. OR-mode for ranked retrieval is the fix. → filed |
...
**What #3 adds beyond #1/#2:** GO is a DAG and GLEIF is a near-acyclic ownership tree — neither could prove the read-path on **genuine cycles**. deps.dev (npm ships circular runtime deps) is the literal reproducer for #29: a real reference cycle commits, reopens, and traverses with no `RecursionError` and **identity preserved through the cycle**. It independently confirms the graph-persona fixes (#29 iterative read, #30 `list[Lazy]`) generalize beyond ownership shape, and it self-fetches from the keyless deps.dev REST API (no manual download).

**What #4 adds beyond #1–#3:** scale — and the first real *gap* an eval has surfaced. The full corpus (6.2 M solar units, ~22 GB XML) ingests at 15.6 k/s and lands the SOR/metadata persona (indexed `Bundesland`/`PLZ`/`Betriebsstatus`, 1.4 ms bitmap counts on 6.2 M rows) and the **#13 multi-valued list index** on genuinely comma-separated real data, both proven against full-scan oracles. **But it falsified the headline hypothesis:** ingest peak RSS is **not** bounded by the commit batch — it reaches **3.9 GB at 6.2 M** because the in-RAM unique/index maps grow with the corpus (the *entity* working set is batch-bounded; the *indexes* are not). Throughput also degrades (18 k → 15.6 k/s) as those maps grow, and cold reopen pays an O(corpus) index rebuild (31.9 s). This is exactly the loop working: a real dataset at real scale turned an assumed property into a measured, actionable gap — see the perf/scale finding (ingest RAM dominated by the unique map; candidates: persist indexes (#12), a bulk-load mode, or accept it as the cost of the `Unique` constraint).

**What #5 adds beyond #1–#4:** a *relevance* oracle. Every prior ground checks correctness by exact-set equality (these OIDs, that descendant set); a search corpus checks **ranking quality** against *human judgments* (qrels) — nDCG@10 / precision@k / recall@k, the thing only a judged dataset can prove and the thing `datacrystal[fts]` exists for. NFCorpus is the fast English wire-up; **MIRACL German** is the headline (the only corpus that exercises the `dc.FullText(language="german")` Snowball path, where a stemming bug becomes a measurable recall loss). And it immediately earned its keep: nDCG@10 0.209 sits **below** the ~0.325 BM25 baseline because `[fts]` AND's query terms — a real, actionable ranking gap a throughput-only eval could never see.
